...
public class Channels implements Serializable {
	...
	private boolean writeChannels() {
		return ReadWrite.write(
				new File(JReader.getConfig().getCacheDir().getPath() + File.separator + "channels"), this.channels);
	}
	...
	private boolean readChannels() {
		this.channels = (Map<String, Channel>) ReadWrite.read(
				new File(JReader.getConfig().getCacheDir().getPath() + File.separator + "channels"));
	...
	private boolean writeItems() {
		return ReadWrite.write(
				new File(JReader.getConfig().getCacheDir().getPath() + File.separator + "items"), this.items);
	}
	...
	private boolean readItems() {
		this.items = (Map<String, Item>) ReadWrite.read(
				new File(JReader.getConfig().getCacheDir().getPath() + File.separator + "items"));
